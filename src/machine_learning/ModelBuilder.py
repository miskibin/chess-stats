...
class ModelBuilder:
    ...
    def __prepare_df(self):
        self.df.insert(
            1, 'elo_gap', self.df['player_elo'] - self.df['opponent_elo'])
        self.df.insert(1, 'hour', self.df['date'].dt.hour)
        self.df.drop(self.df.columns.difference(
            ['player_color', 'elo_gap', 'hour', 'player_result', 'time_class']), 1, inplace=True)
